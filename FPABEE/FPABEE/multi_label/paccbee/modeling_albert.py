        ...
        if self.training:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if i < (self.config.num_hidden_layers - 1):
                    ...
                    encoder_outputs1 = self.encoder.adaptive_forward(encoder_outputs,
                                                                                    current_layer=i + 1,
                                                                                    attention_mask=extended_attention_mask,
                                                                                    head_mask=head_mask
                                                                                    )
                    pooled_output = self.pooler_activation(self.pooler(encoder_outputs[0][:, 0]))
                    pooled_output1 = self.pooler_activation(self.pooler(encoder_outputs1[0][:, 0]))
                    ...
                    #highway_entropy = abs(highway_entropy)
...
class AlbertFormultilabelSequenceClassification(AlbertPreTrainedModel):
    ...
    Returns:
        ...
        outputs = (logits[-1],)
        #print('output_logits.shape',logits.shape)
