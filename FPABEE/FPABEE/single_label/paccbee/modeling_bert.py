...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #self.early_exit_entropy = [0.6 for _ in range(config.num_hidden_layers)]
        #********
    # ********
    # def set_early_exit_entropy(self, x):
    ...
    Examples::
        ...
        #print('np.shape(input_ids)',np.shape(input_ids))
        #shape of input_ids: [32,128]
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds
        )
        #print('np.shape(embedding_output)',np.shape(embedding_output))
        #shape of embedding_output [32, 128, 768]
        #*********
        encoder_outputs = embedding_output
        encoder_outputs1 = embedding_output
        # *********
        # print('np.shape(encoder_outputs)', np.shape(encoder_outputs))
        # shape of encoder_outputs: [32, 128, 768]
        ...
        if self.training:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                #print('np.shape(encoder_outputs)',np.shape(encoder_outputs))
                # shape of encoder_outputs: [32,128,768]
                #print('np.shape(highway_logit)',np.shape(highway_logit))
                # shape of highway_logit: [32,2]
                ########********
                pooled_output = self.pooler(encoder_outputs)
                #print('np.shape(pooled_output)',np.shape(pooled_output))
                # shape of pooled_output: [32, 768]
                logits = output_layers[i](output_dropout(pooled_output))
                #print('np.shape(logits)',np.shape(logits))
                # shape of logits: [32, 2]
                res.append(logits)
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if i < (self.config.num_hidden_layers-1):
                    ...
                    encoder_outputs1 = self.encoder.adaptive_forward(encoder_outputs1,
                                                                    current_layer=i+1,
                                                                    attention_mask=extended_attention_mask,
                                                                    head_mask=head_mask
                                                                    )
                    ...
                    #bi-direction
                    #highway_entropy = -torch.sum(prob * torch.log(prob1), dim=1)-torch.sum(prob1 * torch.log(prob), dim=1)
                    #reversed_direction
                    ...
                    if regression:
                        ...
                        if (patient_result is not None) and torch.abs(
                                patient_result - labels) < self.regression_threshold:
                            patient_counter += 1
                        else:
                            patient_counter = 0
                    ########********
                        ...
                        #print('highway_entropy',highway_entropy)
                        if highway_entropy < self.exiting_threshold:
                            patient_counter += 1
                        else:
                            patient_counter = 0
                    ########********
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        return outputs  # (loss), last_state_logits, (hidden_states), (attentions)
